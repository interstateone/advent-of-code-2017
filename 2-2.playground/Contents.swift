...
func divisiblePair(in numbers: [Int]) -> (min: Int, max: Int)? {
    guard
        // Create pairs with all combinations of numbers
        let pair = numbers.flatMap({ n in
                return zip(
                    Array(repeating: n, count: numbers.count - 1),
                    numbers.filter { $0 != n }
                )
            })
            // Find the first that is evenly divisible
            .first(where: { pair in
                return pair.0 % pair.1 == 0
            })
    else { return nil }

    return (min: min(pair.0, pair.1), max: max(pair.0, pair.1))
}

/// Calculates the difference between the max and min values in each row, then adds them all up
func checksum(of spreadsheet: [[Int]]) -> Int {
    return spreadsheet.reduce(0) { sum, row in
        guard let pair = divisiblePair(in: row) else { return sum }
        return sum + pair.max / pair.min
    }
}

/// Calculates the difference between the max and min values in each row, then adds them all up
/// First, parses a multi-line string into rows by newline characters and row elements by whitespace
func checksum(of spreadsheet: String) -> Int {
    return checksum(of:
        spreadsheet.split(separator: "\n").map { row in
            row.split(separator: " ").flatMap { number in Int(number) }
        }
    )
}
...
ChecksumTest.defaultTestSuite.run()
