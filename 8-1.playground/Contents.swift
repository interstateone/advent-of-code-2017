...
func interpret(program: String) -> Registers {
    ...
    return instructions.reduce([:]) { registers, instruction in
        return instruction.perform(with: registers)
    }
}
...
expect(interpret(program: example).values.max()).to(equal(1))

let inputURL = Bundle.main.url(forResource: "input", withExtension: "txt")!
let input = try! String(contentsOf: inputURL)
interpret(program: input).values.max()
